...
export function WelcomeView() {
  ...
  const startChat = (initialMessage?: string) => {
    createMutation.mutate({ data: { title: "New Conversation" } }, {
      onSuccess: (newConv) => {
        if (initialMessage) {
          setLocation(`/conversations/${newConv.id}?prompt=${encodeURIComponent(initialMessage)}`);
        } else {
  ...
  const suggestions = [
    { label: "Write a Python script to parse CSV files", icon: "⌨️" },
    { label: "Explain quantum computing simply", icon: "⚛️" },
    { label: "Help me design a database schema", icon: "🗄️" },
    { label: "Review this code snippet for bugs", icon: "🔍" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-2xl mb-10">
        {suggestions.map((s, i) => (
          <Button
            key={i}
            variant="outline"
            className="h-auto p-4 justify-start text-left whitespace-normal border-border hover:border-primary/50 hover:bg-primary/5 gap-3"
